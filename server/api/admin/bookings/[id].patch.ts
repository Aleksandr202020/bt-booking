...
export default defineEventHandler(async event=>{
  ...
  if((body.date||body.time||body.carId) && !generateSlots(date).includes(time)) {
         ...
         WHERE id=$6
           AND NOT EXISTS (
             ...
             WHERE booking_date=$1::date
               AND (booking_time=$2::time OR booking_time IS NULL)
           )
           AND NOT EXISTS (
             SELECT 1 FROM bookings other
             WHERE other.booking_date=$1::date
               AND other.booking_time=$2::time
               AND other.id<>$6
               AND other.status NOT IN ('cancelled_customer','cancelled_admin','no_show')
           )
         RETURNING id,user_id,booking_date,booking_time,car_id,price_cents,status`,
        [date,time,carId,price,body.status??null,id]
      )
