...
async function book(){if(!canBook.value||isSlotPast(selectedTime.value))return;error.value='';bookingInProgress.value=true;const car=(cars.value?.cars??[]).find((c:any)=>c.id===selectedCar.value);const price=car?.category==='passenger'?'25':car?.category==='commercial'?'35':'30';try{await $fetch('/api/bookings',{method:'POST',body:{carId:selectedCar.value,date:date.value,time:selectedTime.value}});confirmedBooking.value={date:date.value,time:selectedTime.value,car:car?`${car.make} ${car.model}`:'',price};success.value=true;selectedTime.value='';await loadSlots()}catch(e:any){const status=e?.data?.statusMessage;error.value=status==='BOOKING_DATE_OUT_OF_RANGE'?t('bookingDateRange'):status==='BOOKING_LIMIT_REACHED'?t('bookingLimit'):status==='CAR_BOOKING_LIMIT_REACHED'?t('carBookingLimit'):e?.statusCode===503?t('dbUnavailable'):status==='SLOT_UNAVAILABLE'?t('slotUnavailable'):t('bookingFailed');await loadSlots()}finally{bookingInProgress.value=false}}
...
<div class="mx-auto max-w-2xl">
  ...
  <template v-else>
    ...
    <div class="mt-6 rounded-2xl border bg-white p-5 space-y-5">
      ...
      <p v-if="error" class="rounded-xl bg-red-50 p-3 text-red-700">{{error}}</p><button type="button" :disabled="!canBook||isSlotPast(selectedTime)" class="w-full rounded-xl bg-slate-900 p-3 font-semibold text-white disabled:opacity-40" @click="book">{{t('book')}}</button>
    </div>
  </template>
</div>
</template>
