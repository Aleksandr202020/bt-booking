...
async function book(){if(!canBook.value)return;error.value='';bookingInProgress.value=true;const car=(cars.value?.cars??[]).find((c:any)=>c.id===selectedCar.value);const price=car?.category==='passenger'?'25':car?.category==='commercial'?'35':'30';try{await $fetch('/api/bookings',{method:'POST',body:{carId:selectedCar.value,date:date.value,time:selectedTime.value}});confirmedBooking.value={date:date.value,time:selectedTime.value,car:car?`${car.make} ${car.model}`:'',price};success.value=true;selectedTime.value='';await loadSlots()}catch(e:any){error.value=e?.statusCode===503?t('dbUnavailable'):e?.data?.statusMessage==='SLOT_UNAVAILABLE'?t('slotUnavailable'):t('bookingFailed');await loadSlots()}finally{bookingInProgress.value=false}}
...
<div class="mx-auto max-w-2xl">
  ...
  <template v-else>
    ...
    <div class="mt-6 rounded-2xl border bg-white p-5 space-y-5">
      ...
      <div><label class="font-semibold">{{t('date')}}</label><input v-model="date" type="date" class="mt-2 w-full rounded-xl border p-3" @change="loadSlots"></div>
      <div><div class="flex items-center justify-between"><label class="font-semibold">{{t('freeTime')}}</label><button v-if="date" type="button" class="text-sm underline" @click="loadSlots">{{t('retry')}}</button></div><div class="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-4"><button v-for="slot in slots" :key="slot" type="button" class="rounded-xl border p-3" :class="selectedTime===slot?'bg-slate-900 text-white':''" @click="selectedTime=slot">{{slot}}</button></div><p v-if="loadingSlots" class="mt-2 text-sm text-slate-500">{{t('checkDb')}}</p><p v-if="!loadingSlots&&date&&!slots.length&&!error" class="mt-2 text-sm text-slate-500">{{t('noSlots')}}</p></div>
      <p v-if="error" class="rounded-xl bg-red-50 p-3 text-red-700">{{error}}</p><button type="button" :disabled="!canBook" class="w-full rounded-xl bg-slate-900 p-3 font-semibold text-white disabled:opacity-40" @click="book">{{t('book')}}</button>
    </div>
  </template>
</div>
</template>
