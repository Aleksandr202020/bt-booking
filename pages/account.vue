...
const editingId=ref(''); const editDate=ref(''); const editTime=ref(''); const editCar=ref(''); const editSlots=ref<string[]>([]); const editLoading=ref(false); const editSaving=ref(false)
...
async function saveEdit(){
  ...
  try{
    ...
    error.value=e?.data?.statusMessage==='SLOT_UNAVAILABLE'?t('slotUnavailable'):e?.data?.statusMessage==='BOOKING_CANNOT_BE_EDITED'?t('bookingFailed'):e?.statusCode===503?t('dbUnavailable'):t('bookingFailed')
    await loadEditSlots()
